...
async def test_osi(session, url, method, headers=None, data=None):
    ...
    try:
        ...
        for payload in OSI_PAYLOADS:
            if re.search(r'(uid|gid|groups)=[0-9]+\(.+?\)|[a-zA-Z_-]{1,}:x:[0-9]{1,}:[0-9]{1,}:', response.text):  # Check for passwd file
                return {
                    "url": url,
                    "method": method,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="results_osi_linux.json", help="Output file for results (default: results_osi_linux.json)")
